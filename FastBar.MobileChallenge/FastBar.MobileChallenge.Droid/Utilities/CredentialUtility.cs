using FastBar.Mobile.Droid.Helpers;
...
namespace FastBar.MobileChallenge.Droid.Utilities
{
    internal class CredentialUtility : ICredentialUtility
    {
        ...
        {
            var username = _settingsUtility.Read<string>(UsernamePrefix + resource);
            var password = _settingsUtility.Read<string>(PasswordPrefix + resource);
